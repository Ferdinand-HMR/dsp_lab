...
y[0, y.shape[1] - R / 2 + 0 : y.shape[1] - R / 2 + R / 2 ] = y[0, y.shape[1] - R / 2 + 0 : y.shape[1] - R / 2 + R / 2 ] /( window[0, R / 2  : window.shape[1]] ** 2 )
...
X = stft(x,R,Nfft)
N = x.shape[1]
inverse = stft_inverse(X,R,N)

error = x - inverse

error_sum = np.sum(error)
print "reconctruction error: ", error_sum